...
def test_headless_mode_readme_line_count_no_browser():
    ...
    try:
        ...
        while time.time() - start_time < max_wait_time:
            ...
            if (
                conversation.get('status') == 'finished'
                or conversation.get('status') == 'stopped'
            ):
                break
        ...
        response = requests.get(
            f'{base_url}/api/conversations/{conversation_id}/messages', timeout=10
        )
        ...
        for message in messages:
            if message.get('source') == 'agent':
                ...
                if any(
                    str(count) in content for count in [expected_line_count, 157, 183]
                ) and (
                    'line' in content.lower()
                    or 'count' in content.lower()
        ...
        if not found_count:
            ...
            pytest.fail(
                f'Line count not found in agent responses. Expected around {expected_line_count}. Agent responses: {all_responses}'
            )

        # Get the conversation events to check for browsing actions
        response = requests.get(
            f'{base_url}/api/conversations/{conversation_id}/events', timeout=10
        )
        ...
        if response.status_code == 200:
            ...
            if browsing_actions_found:
                pytest.fail(
                    'Browsing actions were found in the conversation, but this test should not use browser'
                )
        ...
        print(
            '✓ Test passed: README.md line count found and no browsing actions detected'
        )
